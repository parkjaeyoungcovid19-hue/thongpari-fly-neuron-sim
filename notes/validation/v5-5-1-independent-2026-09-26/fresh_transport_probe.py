...
results = []
for mode, test in [('--mock', 'bridgeloop'), ('--mock', 'labloop'), ('--mock', 'v4loop'), ('--flygym-headless', 'labloop'), ('--flygym-headless', 'v4loop')]:
    ...
    with (out / f'fresh-{tag}-backend.log').open('w') as log:
        ...
        try:
            ...
            with (out / f'fresh-{tag}.log').open('w') as testlog:
                result = subprocess.run(
                    ['./ThongpariFlyNeuronSim', '--' + test], stdout=testlog,
                    stderr=subprocess.STDOUT, timeout=100)
            results.append(dict(mode=mode, test=test, exit=result.returncode))
